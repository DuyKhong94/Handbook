...
st.sidebar.image("https://raw.githubusercontent.com/DuyKhong94/41f3521519501a81dbb602aae0449705f781fd77/8c36775f-19bb-4cc5-b3ef-b7e3cd54b3b8.png",use_container_width=True)
st.sidebar.markdown("---")
...
# # ==========================================================
   ...
   #body of email
    ...
    axs[1,0].set_ylim(0, max(df_eol["EOL Cases"]) + 200 )
    ...
    axs[1,2].set_ylim(0,max(df_mainfactors["Factors"])+100)
    axs[1,2].tick_params(axis='x', labelsize=6)


    #RANK OF MODEL
    df15 = df1[
        df1["Product"].astype(str).str.startswith(tuple(models))
    ].copy()

    df15["Product"] = df15["Product"].astype(str).str[:9]
    ...
    axs[2,2].set_ylim(0,max(df_mat_sum["Value"])+50)
    ...
    axs[2,1].set_ylim(0,max(df_linedown_resolved["manhours"])+500)
    ...
    axs[2,0].xaxis.set_major_locator(mdates.DayLocator(interval=2))   # 👈 mỗi 3 ngày
